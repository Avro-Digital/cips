...
### 4.8 Reward attribution

Coupon rewards generated by a lock are attributed to the SV or FA beneficiary recorded on its `ActiveLockContext` (per §4.3.1). Building on existing Canton Coin minting and distribution logic, the primitive supports two reward-allocation behaviors:

- **Allocation across multiple beneficiaries.** A single coupon allocation can be split across multiple beneficiary parties. The split policy is set at the application layer; the primitive provides the attribution data required to compute it per §4.7.2.
- **Allocation into locked and liquid states.** Minting actions that distribute rewards from a lock route the resulting CC into Locked and Liquid states in configured proportions. CC routed to Locked is created together with an associated `ActiveLockContext` per §4.2.2; CC routed to Liquid is created as an ordinary Liquid token.
...
- 2026-05-26: Review feedback. Moved Source documents subsection out of §3 (consolidated under §11 References). Reworded §4.3.2 to frame the beneficiary-type storage question as implementation discretion rather than an open working-group item, and removed the corresponding entry from §10 (renumbered remaining items). Updated cross-reference in §6.5 from §3 to §11. Rewrote §4.8 (Reward attribution) to assert two supported reward-allocation behaviors — allocation across multiple beneficiaries, and allocation into Locked and Liquid states — and dropped the WIP framing.
